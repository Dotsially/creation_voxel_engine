...
World::World(){
    ...
    for(i32 x = 0; x < REGION_SIZE; x++){
        for(i32 z = 0; z < REGION_SIZE; z++){
            ...
            chunks[z+x*REGION_SIZE].CreateChunkData(&noise, x,z);
        }
    }
...
bool World::IsWithinWorld(i32 x, i32 y, i32 z){
    if(x >= 0 && x < CHUNK_SIZE * REGION_SIZE && y >= 0 && y < CHUNK_SIZE && z >= 0 && z < CHUNK_SIZE * REGION_SIZE){
        return true;
    }
    return false;
}

bool World::ContainsBlock(i32 x, i32 y, i32 z){
...
void World::RemoveBlock(i32 x, i32 y, i32 z){
    ...
    Chunk* neighbors[4];
    GetNeighbors(neighbors, chunkCoordinate.x, chunkCoordinate.y);
    chunks[chunkCoordinate.y + chunkCoordinate.x*REGION_SIZE].RemoveBlock(neighbors, x%32, y, z%32);
